...
- 如果浏览器限制本地网页运行，直接双击 `启动网页版.bat`，它会使用 Dart 内置
  服务器启动网页，确认服务就绪后再打开浏览器，不需要 Python，也不需要输入命令。
- Microsoft Edge 如果仍显示旧启动页，请双击 `启动网页版-Edge.bat`。该入口使用
  独立端口 `18766` 和专用的永久浏览器档案，与旧的 Service Worker 缓存完全隔离。
  学习进度、每日目标、收藏、设置和自定义词包会保存在
  `%LOCALAPPDATA%\Qiuzhi\EdgeProfile`，关闭 Edge 或重启电脑后仍会保留。
- 普通浏览器入口同样使用浏览器本地存储。请保持访问地址和端口不变；清除该网站的
  浏览数据会同时清除本地学习进度。
